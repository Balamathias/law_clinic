...
from app.serializers import RegisterSerializer, UserSerializer, TokenObtainPairSerializer, HelpRequestSerializer
...
from app.permissions import (IsAdminOrReadOnly, IsOwnerOnly, IsOwnerOrReadOnly)
...
class RequestPasswordResetView(APIView, ClinicView):
    ...
    def post(self, request):
        email = request.data.get("email")
        if not email:
            return self.clinic_response(error="Email is required.", message="Email is required.", status=status.HTTP_400_BAD_REQUEST)
        try:
            user = User.objects.get(email=email)
        except User.DoesNotExist:
            return self.clinic_response(error="User not found.", message="No user with this email.", status=status.HTTP_404_NOT_FOUND)
        ...
        return self.clinic_response(message="OTP sent to your email.", status=status.HTTP_200_OK)


class VerifyPasswordResetOTPView(APIView, ClinicView):
    permission_classes = (AllowAny,)
    authentication_classes = ()
    ...
    def post(self, request):
        ...
        if not (email and otp and new_password):
            return self.clinic_response(error="Missing fields.", message="Email, OTP, and new password are required.", status=status.HTTP_400_BAD_REQUEST)
        try:
            user = User.objects.get(email=email)
        except User.DoesNotExist:
            return self.clinic_response(error="User not found.", message="No user with this email.", status=status.HTTP_404_NOT_FOUND)
        if not default_token_generator.check_token(user, otp):
            return self.clinic_response(error="Invalid OTP.", message="The OTP is invalid or expired.", status=status.HTTP_400_BAD_REQUEST)
        user.set_password(new_password)
        user.save()
        return self.clinic_response(message="Password reset successful.", status=status.HTTP_200_OK)
...
class LogoutView(APIView, ClinicView):
    ...
    def post(self, request):
        try:
            ...
            return self.clinic_response(data={"message": "Logout successful"}, message="Logout successful", status=status.HTTP_205_RESET_CONTENT)
        except Exception as e:
            return self.clinic_response(data={"message": "Bad request"}, message="Bad request", status=status.HTTP_400_BAD_REQUEST)
