...
class RequestPasswordResetView(APIView, ClinicView):
    ...
    def post(self, request):
        email = request.data.get("email")
        if not email:
            return self.clinic_response(error="Email is required.", message="Email is required.", status_code=status.HTTP_400_BAD_REQUEST)
        try:
            user = User.objects.get(email=email)
        except User.DoesNotExist:
            return self.clinic_response(error="User not found.", message="No user with this email.", status_code=status.HTTP_404_NOT_FOUND)
...
class ValidateResetTokenView(APIView, ClinicView):
    ...
    def post(self, request, uid, token):
        try:
            ...
            return self.clinic_response(error="Invalid link.", message="The password reset link is invalid or expired.", status_code=status.HTTP_400_BAD_REQUEST)
            
        if default_token_generator.check_token(user, token):
            return self.clinic_response(message="Token is valid.", status_code=status.HTTP_200_OK)
        return self.clinic_response(error="Invalid or expired token.", message="The password reset link is invalid or expired.", status_code=status.HTTP_400_BAD_REQUEST)
...
class ConfirmPasswordResetView(APIView, ClinicView):
    ...
    def post(self, request, uid, token):
        ...
        if not password:
            return self.clinic_response(error="Password is required.", message="Password is required.", status_code=status.HTTP_400_BAD_REQUEST)
